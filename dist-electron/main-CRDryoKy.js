...
function handleFileAdd(filePath) {
  const existing = getMovies().find((m) => m.file_path === filePath);
  if (existing) return;
...
function registerIPC() {
  ...
  ipcMain.handle("db:generate-default-playlists", async () => {
    const { generateDefaultPlaylists } = await import("./playlists-B0hM5FEX.js");
  ...
  ipcMain.handle("media:get-metadata", async (_, filePath) => {
    const { getMediaMetadata } = await import("./ffmpeg-CEQRh3zy.js");
    return getMediaMetadata(filePath);
  });
  ipcMain.handle("media:extract-subtitle", async (_, filePath, trackIndex) => {
    const { extractSubtitle } = await import("./ffmpeg-CEQRh3zy.js");
