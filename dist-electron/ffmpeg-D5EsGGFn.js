...
async function extractSubtitle(filePath, trackIndex) {
  const tempDir = app.getPath("temp");
  const outputPath = sysPath__default.join(tempDir, `kino_sub_${sysPath__default.basename(filePath)}_${trackIndex}.vtt`);
  return new Promise((resolve, reject) => {
    ffmpeg(filePath).output(outputPath).outputOptions([
      `-map 0:${trackIndex}`,
      "-f webvtt",
      "-y"
    ]).on("end", () => resolve(outputPath)).on("error", (err) => reject(err)).run();
  });
}
async function extractSubtitleContent(filePath, trackIndex) {
